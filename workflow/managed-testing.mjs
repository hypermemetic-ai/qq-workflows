...
import { existsSync, mkdirSync, mkdtempSync, readFileSync, readlinkSync, realpathSync, renameSync, rmSync, writeFileSync, readdirSync, statSync } from 'node:fs';
...
export function initManagedTesting({ stateDir, id, root, worktree, ticket }) {
  const state = { schema: 1, root, worktree, stateDir, plan: testingPlan(ticket), selection: null,
...
function config(state) {
  const data = JSON.parse(readFileSync(join(state.worktree, '.architect', 'test-runner.json'), 'utf8'));
  if (data.schema !== 1 || data.command !== 'node' || !Array.isArray(data.args) || data.args.length !== 0 ||
      typeof data.directory !== 'string' || !/^[a-zA-Z0-9_/-]+$/.test(data.directory) || data.directory.includes('..') ||
      typeof data.extension !== 'string' || !/^\.[a-zA-Z0-9]+$/.test(data.extension))
    throw new Error('unsupported project focused-test runner configuration');
  return data;
}
function targetPath(state, cfg, target) {
  if (typeof target !== 'string' || !/^[a-zA-Z0-9_./-]+$/.test(target) || target.includes('..') || target.startsWith('/') || target === 'run.mjs') throw new Error(`invalid focused target: ${target}`);
  ...
  if (!path.startsWith(dir + sep) || !target.endsWith(cfg.extension) || !existsSync(path) || !realpathSync(path).startsWith(realpathSync(dir) + sep) || !statSync(path).isFile()) throw new Error(`not an existing focused test: ${target}`);
...
export function selectManagedTests(binding, { targets, rationale }) {
  ...
  state.selection = { targets, rationale, by: binding.role, at: Date.now() };
...
function launch(command, args, cwd, timeoutMs, signal) {
...
export async function runManagedTests(binding, { signal, expectedRed = null } = {}) {
  ...
  if (!state.selection?.targets?.length) {
    const record = {role:binding.role,round:state.round,selection:[],hash:workingState(state),outcomes:[],status:'no-tests',at:Date.now()};
  ...
  const cfg = config(state), hash = workingState(state), outcomes = [];
  for (const target of state.selection.targets) {
    const file = targetPath(state, cfg, target);
    outcomes.push({ target, ...await launch(cfg.command, [...cfg.args, file], state.worktree, 120000, signal) });
  ...
  const record = { role: binding.role, round: state.round, selection: [...state.selection.targets], hash, outcomes, status,
...
export function focusedPass(state) {
  ...
  return Boolean(latest && latest.status === 'pass' && latest.hash === workingState(state) &&
...
export async function runManagedCheckpoint(binding, { signal } = {}) {
  ...
  if (!focusedPass(state) || !state.reviews.some(r => r.round === state.round && r.verdict === 'READY' && r.hash === workingState(state)))
  ...
  const hash = workingState(state), entry = { round:state.round, hash, selection:[...state.selection.targets], command:state.plan.command, status:'incomplete', at:Date.now() };
...
export function recordManagedReview(binding, { verdict, implementation = [], tests = [], selection = [], decision = null, incomplete = [], suggestions = [] }) {
  ...
  if ((verdict === 'PASS' || verdict === 'READY') && (!focusedPass(state) ||
      (verdict === 'PASS' && state.plan.required && !state.checkpoints.some(c => c.round === state.round && c.status === 'pass' && c.hash === workingState(state) && JSON.stringify(c.selection) === JSON.stringify(state.selection.targets)))))
  ...
  const review = { verdict, groups, ...(decision ? {decision} : {}), round:state.round, hash:workingState(state), at:Date.now() };
...
export function managedLandingReady(stateDir,id) {
  ...
  return Boolean(last?.verdict === 'PASS' && last.round === state.round && last.hash === workingState(state) && focusedPass(state) &&
    (!state.plan.required || state.checkpoints.some(c => c.round === state.round && c.status === 'pass' && c.hash === last.hash && JSON.stringify(c.selection) === JSON.stringify(state.selection.targets))));
}
