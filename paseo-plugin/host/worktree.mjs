...
export function parseWorkspaceJson(text) {
  ...
  if (start < 0 || end <= start) throw new Error("paseo workspace create produced no json");
  const parsed = JSON.parse(raw.slice(start, end + 1));
  const cwd = parsed.cwd ?? parsed.workspaceDirectory ?? parsed.directory ?? null;
  const workspaceId = parsed.workspaceId ?? parsed.id ?? null;
  if (!isRealCwd(cwd)) throw new Error("paseo workspace create missing cwd");
