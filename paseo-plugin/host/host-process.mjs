...
import { sendAgentWake } from './spawn-agent.mjs';
import { join } from 'node:path';
const runtime = createRuntime({
  storePath: join(STATE_DIR, 'state.sqlite'),
  sendWake: (agentId, text) => sendAgentWake(agentId, text).catch(err => console.error("Failed to send wake", err)),
  onIdleUpgrade: async () => { await runtime.close(); process.exit(0); }
});
