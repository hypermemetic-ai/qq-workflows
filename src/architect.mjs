...
export const ARCHITECT_PROMPT = [
  ...
  "Work autonomously on routine in-domain actions within the standing `workspace-write` sandbox; omit permission fields from commands that only write inside the workspace.",
  "When a command needs an external write, declare only its exact narrow directory roots with bash `writable_paths`. The host canonicalizes them, silently reuses grants for the logical project, and remembers newly approved folder grants; rejection or cancellation denies only the current request and does not prevent a later request.",
  "If the host refuses `/`, the home root, the projects-container root, or another protected broad ancestor, ask the operator whether that exact broad folder is truly needed and normally retry with a narrower child folder. Use `danger-full-access` only as rare break-glass behavior, never for routine package caches, data folders, or sibling tool directories expressible with `writable_paths`.",
