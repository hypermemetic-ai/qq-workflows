...
import { callHost } from '../paseo-plugin/host/host-client.mjs';
import { createResponsesAccumulator, completeArchitect } from '../paseo-plugin/host/providers/model.mjs';
...
try {
  ...
  const hostUrl = await runtime.listen();
  try {
    assert.equal((await callHost('/health', {}, { hostUrl })).ok, true);
    assert.deepEqual((await callHost('/wakes', { agentId: 'architect' }, { hostUrl })).wakes, runtime.peekWakes('architect'));
    await assert.rejects(callHost('/tool', {
      name: 'delegate', arguments: {}, context: { role: 'teacher', cwd: dir },
    }, { hostUrl }), error => /cannot execute/.test(error.message) && typeof error.failureClass === 'string');
    const cancelled = AbortSignal.abort(new Error('cancelled host request'));
    await assert.rejects(callHost('/wakes', {}, { hostUrl, signal: cancelled }), /cancelled host request/);
