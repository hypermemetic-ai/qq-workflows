...
// and hides the await tools from Astra's schema.
{
  ...
  assert.ok(!fresh.includes("web_search = false"));
  assert.ok(fresh.includes('disabled_tools = ["wait", "sleep", "web__run", "web", "web_search"]'));
  ...
  assert.ok(merged.includes('web_search = "disabled"'));
  assert.ok(!merged.includes("web_search = false"));
  assert.ok(merged.includes('disabled_tools = ["wait", "sleep", "web__run", "web", "web_search"]'));
...
// 12. Codex Shim: full launch against a fake codex.
{
  ...
  try {
    ...
    assert.ok(call.includes('disabled_tools=["wait","sleep","web__run","web","web_search"]'), "codex-architect must pass disabled_tools");
    assert.ok(call.includes('web_search="disabled"'), 'codex-architect must pass web_search="disabled"');
    assert.ok(!call.includes("features.web_search"), "codex-architect must not pass deprecated features.web_search");
...
// 14. installAgents cleans muse auth and configures Orca files
{
  ...
  try {
    ...
    assert.ok(tomlContent.includes('web_search = "disabled"'));
    assert.ok(!tomlContent.includes("web_search = false"));
    assert.ok(tomlContent.includes('disabled_tools = ["wait", "sleep", "web__run", "web", "web_search"]'));
